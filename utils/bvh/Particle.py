            ...
            if particle.collision_enabled:
                ...
                while is_colliding:
                    ...
                    for tmp_collider in self.colliders:
                        ...
                        if tmp_is_colliding:
                            tmp_distance_to_collision = algebra.distance_of(particle.position_prev, tmp_collision_point)
...
class Damped_Spring_Force(Force):
    def __init__(self, p: Particle, system: Particle_System, p2: Particle,
        ...
        self.r: float = r
        if self.r is None:
            self.r = algebra.distance_of(p.position, p2.position)

    @override
    def apply(self):
        current_distance = algebra.distance_of(self.p.position, self.p2.position)
        dv = self.p2.velocity - self.p.velocity
        dx = self.p2.position - self.p.position
        dx_unit = algebra.numpy_get_unit(dx)
        
        dv_size = np.linalg.norm(algebra.decompose_by(dv,dx))
        
        f = self.ks * (current_distance - self.r) * dx_unit + self.kd * algebra.decompose_by(dv, dx)
...
class Infinite_Plane_Collider(Collider):
    def __init__(self, 
        ...
        self.norm: np.ndarray = algebra.numpy_get_unit(normal_vector)
    ...
    def apply_collision(self, particle: Particle, collision_point: np.ndarray):
        ...
        decomposed_movement = algebra.decompose_by(particle.position - collision_point, self.norm)
        decomposed_velocity = algebra.decompose_by(particle.velocity, self.norm)
    
        particle.position_prev = collision_point - self.eps * self.eps * algebra.numpy_get_unit(decomposed_movement)
    ...
    def apply_contact(self, particle: Particle, dt: float):
        # apply contact force
        down_force = algebra.decompose_by(particle.force, self.norm)
        passpoint_to_particle = particle.position - self.passpoint
        up_direction = algebra.decompose_by(passpoint_to_particle, self.norm)

        parallel_force = particle.force - down_force

        down_velocity = algebra.decompose_by(particle.velocity, self.norm)
        ...
        if np.dot(down_force, up_direction) < 0: 
            ...
            if np.linalg.norm(parallel_velocity) < self.eps * self.myu:
                # particle.velocity = algebra.decompose_by(particle.velocity, self.norm)
            ...
            if np.linalg.norm(parallel_velocity) > 0:
                friction_for_velocity = self.myu * np.linalg.norm(down_force) * algebra.numpy_get_unit(parallel_velocity)
            ...
            #     friction_for_current_force = self.myu * np.linalg.norm(down_force) * algebra.numpy_get_unit(particle.force)
    ...
    def check_contact(self, particle: Particle, dt:float) -> bool:
        ...
        velocity_toward_plane = np.linalg.norm(algebra.decompose_by(particle.velocity, self.norm))
